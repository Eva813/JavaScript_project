...
function deleteBookmark(url){
	...
	bookmarks.forEach((bookmark, i)=>{
		if(bookmark.url === url){
			bookmarks.splice(i,1); //當有多個url，每次都要for loop 所有item
		}
	});
...
function buildBookmarks (){
  ...
  //建立item
    ...
    const {name,url} = bookmark; //透過將物件解構取得裡面的資料
    ...
    const item = document.createElement('div');
		...
		delIcon.setAttribute('onClick',`deleteBookmark('${url}')`); //刪除事件
...
function getBookmarksArr (){
	...
	if(localStorage.getItem('bookmarks')){
		...
		bookmarks = [{
			name: 'John',
			url: 'https://developer.mozilla.org/zh-TW/docs/Web/API/Window/localStorage'
		}]
		localStorage.setItem('bookmarks', JSON.stringify(bookmarks) );
	}
  buildBookmarks();
